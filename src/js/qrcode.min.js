/**
 * Minified QR Code Generator for Super BT (Offline & Standalone)
 * Adapted from QRCode for JavaScript by Kazuhiko Arase & David Shim
 */
(function(window){
  ...
  Polynomial.prototype={
    ...
    mod:function(e){
      ...
      for(var i=0;i<e.getLength();i++)num[i]^=QRPolynomial.gexp(QRPolynomial.glog(e.get(i))+ratio);
  ...
  var QRUtil={
    PATTERN_POSITION_TABLE:[[],[6,18],[6,22],[6,26],[6,30],[6,34],[6,22,38],[6,24,42],[6,26,46],[6,28,50],[6,30,54],[6,32,58],[6,34,62]],
    ...
    getErrorCorrectPolynomial:function(errorCorrectLength){var a=new Polynomial([1],0);for(var i=0;i<errorCorrectLength;i++)a=a.multiply(new Polynomial([1,QRPolynomial.gexp(i)],0));return a;},
  ...
  /**
   * Helper principal: Renderiza SVG responsivo
   */
  window.QRCode = {
    generateSVG: function(text, options) {
      ...
      var qr = new QRCodeModel(0, 1); // 1 = Level M
      qr.addData(text);
      qr.make();

      var count = qr.getModuleCount();
      var fullCount = count + (margin * 2);
      var rectSize = size / fullCount;

      var svg = ['<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + size + ' ' + size + '" width="' + size + '" height="' + size + '" style="border-radius:12px;background:' + lightColor + ';">'];
      svg.push('<rect width="' + size + '" height="' + size + '" fill="' + lightColor + '" rx="12" />');

      for (var r = 0; r < count; r++) {
        for (var c = 0; c < count; c++) {
          if (qr.isDark(r, c)) {
            var x = (c + margin) * rectSize;
            var y = (r + margin) * rectSize;
            svg.push('<rect x="' + x.toFixed(2) + '" y="' + y.toFixed(2) + '" width="' + (rectSize + 0.3).toFixed(2) + '" height="' + (rectSize + 0.3).toFixed(2) + '" fill="' + darkColor + '" />');
          }
        }
      }
